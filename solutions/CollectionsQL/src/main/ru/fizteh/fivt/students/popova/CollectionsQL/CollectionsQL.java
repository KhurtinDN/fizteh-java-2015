 ...
 */
public interface CollectionsQL<T>{
    ...
    public static <T>Stream<T> where( Stream<T> myTable, Predicate<T> p  ){
            List<T> returnList = new ArrayList<T>();
            myTable.filter(s -> p.test(s))
                    .forEach(s -> returnList.add(s));
            return null;
    }
    ...
    public static <T> void having(Stream<T> myTable, Predicate<T> p){
        myTable.
                filter(s->p.test(s));
    }
