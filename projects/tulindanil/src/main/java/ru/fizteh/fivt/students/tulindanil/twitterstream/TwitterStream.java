package ru.fizteh.fivt.students.tulindanil.twitterstream;

/**
 * Created by tulindanil on 17.09.15.
 */

import com.beust.jcommander.*;
import ru.fizteh.fivt.students.tulindanil.twitterstream.library.*;
...
import ru.fizteh.fivt.students.tulindanil.twitterstream.library.exceptions.TwitterStreamException;
import twitter4j.*;
...
public class TwitterStream {
    ...
    static ru.fizteh.fivt.students.tulindanil.twitterstream.library.Location resolveLocation(String passedLocation)
            throws LocationDefinitionErrorException, InvalidLocationException,
            MalformedURLException {
        ru.fizteh.fivt.students.tulindanil.twitterstream.library.Location result = null;
    ...
    public static void main(String[] args) throws TwitterException {
        ...
        try {
            ...
            JCommander jCommander = new JCommander(jCommanderSettings, "--query", "query");
            jCommander.usage();
            return;
        }

        ru.fizteh.fivt.students.tulindanil.twitterstream.library.Location currentLocation = null;
        try {
            currentLocation = resolveLocation(jCommanderSettings.getLocation());
        } catch (LocationDefinitionErrorException | InvalidLocationException | MalformedURLException e) {
            System.err.println(e.getMessage());
            System.exit(1);
        }
        ...
        tweetsPrinter.print("Твиты по запросу " + jCommanderSettings.getQuery() + " для " + currentLocation.getName());

        if (!jCommanderSettings.isStream()) {
            try {
                tweetsPrinter.printTweets(tweetsGetter.getTweetsOnce(jCommanderSettings, currentLocation,
            ...
            tweetsGetter.launchTwitterStream(jCommanderSettings, currentLocation, tweetsPrinter::print,
