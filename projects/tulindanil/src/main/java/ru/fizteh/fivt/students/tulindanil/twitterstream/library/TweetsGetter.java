...
public class TweetsGetter {
    ...
    public List<String> getTweetsOnce(JCommanderSetting jCommanderSettings, Location currentLocation, Twitter twitter)
        ...
        Query query = new Query(jCommanderSettings.getQuery()).geoCode(new GeoLocation(currentLocation.getLatitude(),
                        currentLocation.getLongitude()), RADIUS, RADIUS_UNIT);
        query.setCount(jCommanderSettings.getLimit());

        for (int numberOfTry = 0; numberOfTry < PlaceLocationResolver.MAX_NUMBER_OF_TRIES; ++numberOfTry) {
            try {
                return twitter.search(query).getTweets().stream().filter(e -> !e.isRetweet()
                        || !jCommanderSettings.isHideRetweets()).map(Formatter::formatTweet).collect(toList());
            } catch (TwitterException e) {
                if (numberOfTry + 1 == PlaceLocationResolver.MAX_NUMBER_OF_TRIES) {
                    throw new TwitterStreamException("Error while search", e);
                }
            }
        }
        throw new TwitterStreamException("Unexpected end of " + "TweetsOnce function");
    }

    public void launchTwitterStream(JCommanderSetting jCommanderSetting,
        ...
        twitterStream.addListener(new StatusAdapter() {
            ...
            public void onStatus(Status status) {
                ...
                if (SphereDistanceResolver.sphereDistance(
                                tweetLocation.getLatitude(),
        ...
        twitterStream.filter(new FilterQuery().track(jCommanderSetting.getQueries().toArray(
